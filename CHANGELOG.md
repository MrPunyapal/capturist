...
All notable changes to **`sitesnap`** will be documented in this file.
...
- **Configuration System**:
  ...
  - Multi-format configuration auto-discovery (`sitesnap.config.ts`, `sitesnap.config.mjs`, `sitesnap.config.js`, `sitesnap.config.cjs`, `sitesnap.config.json`).
...
- **Command-Line Interface (CLI)**:
  - `sitesnap` binary with commands for generation, configuration initialization (`sitesnap init`), custom configs (`--config`), base URL overrides (`--baseUrl`), output directories (`--outputDir`), concurrency (`--concurrency`), and dry runs (`--dry-run`).
