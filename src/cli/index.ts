...
function printJsonSummary(summary: RunSummary): void {
  const payload = {
    ok: summary.failed === 0,
    total: summary.total,
...
export async function runCli(argv: string[] = process.argv.slice(2)): Promise<void> {
  ...
  try {
    ...
    if (options.dryRun) {
      if (!quiet) {
        logger.info(
          `Dry run mode: validated ${config.pages.length} page targets without launching browser.`
        );
        config.pages.forEach((p, i) => {
          logger.info(
            ` [${i + 1}/${config.pages.length}] ${resolvePageLabel(p)} → ${p.output}`
          );
        });
      }
      if (options.json) {
