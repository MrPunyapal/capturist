 ...
 */
export function resolveConfigFile(cwd, customPath) {
    if (customPath) {
...
export async function loadConfigFile(configPath) {
    ...
    if (ext === ".json") {
        ...
        try {
            ...
            const tmpFile = path.resolve(path.dirname(configPath), `.capturist.tmp.${Date.now()}.mjs`);
            try {
...
export async function loadConfig(cwd = process.cwd(), customPath) {
    ...
    if (!resolvedPath) {
        throw new Error(`No capturist configuration file found in "${cwd}". Create a "capturist.config.ts" or "capturist.config.js" or use "capturist init".`);
    }
