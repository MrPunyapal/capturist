 ...
 */
export async function startStaticServer(serverConfig, baseDir = process.cwd()) {
    const rootDir = path.resolve(baseDir, serverConfig.dir);
    if (!fsSync.existsSync(rootDir)) {
        throw new Error(`Static server directory does not exist: "${rootDir}". Did you build your website first (e.g. npm run build)?`);
    }
